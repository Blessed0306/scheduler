...
def main():
    ...
    for i in range(0, num_processes):
        if data_set[i].io_frequency == 0:
            for j in range(data_set[i].duration):
                output = output + data_set[i].name + " "
                
        
        else:
            count = data_set[i].io_frequency
            for j in range(0, data_set[i].duration):
                output = output + data_set[i].name + " "
               
                if j == data_set[i].io_frequency:
                    output = output + "!" + data_set[i].name + " "
                    
                    data_set[i].io_frequency +=count
